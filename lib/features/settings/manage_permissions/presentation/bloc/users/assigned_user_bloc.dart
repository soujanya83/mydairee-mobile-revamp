...
class AssignerPermissionUserBloc extends Bloc<AssignerPermissionUserEvent, AssignerPermissionUserState> {
  ...
  Future<void> _onFetchUsersAndPermissions(
      FetchUsersAndPermissions event, Emitter<AssignerPermissionUserState> emit) async {
    emit(AssignerPermissionUserLoading());
    try {
      final users = await repository.fetchUsers();
      final permissions = await repository.fetchPermissions();
      emit(AssignerPermissionUserLoaded.AssignerPermissionUserLoaded(users, permissions));
    } catch (e) {
  ...
  Future<void> _onUpdateUserPermissions(
      UpdateUserPermissions event, Emitter<AssignerPermissionUserState> emit) async {
    try {
      await repository.updateUserPermissions(event.userId, event.permissions);
      final users = await repository.fetchUsers(); // Refresh user list
      final permissions = await repository.fetchPermissions();
      emit(AssignerPermissionUserLoaded.AssignerPermissionUserLoaded(users, permissions));
    } catch (e) {
